...
df_counts = df.groupby(['user', 'session', 'block_id', 'block', 'condition'], observed=True).size()\
            .rename('valid trials count').reset_index()
...
projections = df.groupby(['user', 'session', 'task'], observed=True)[['df1', 'df2']].apply(analysis.get_projections,
                                                                                           ucm_vec)
